...
std::vector<int> Arrays::generateArray() {
    if (this->params.size <= 0 || this->params.min > this->params.max || this->params.min < 0 ||
        this->params.max < 0) {
    ...
    for (auto& num : generatedArray) num = dist(gen);
...
void Arrays::displayArray(const std::vector<int>& arr, const std::string& name) const {
    std::cout << RED << name << RESET << " array: [";

    for (int num : arr) {
        std::cout << GREEN << num << RESET << " ";
    }
